...
## 2. `Google Auth Platform`/Auth权限请求页面配置

### 1. Get Started

- 点击 `APIs & Services` → `OAuth consent screen`，配置 `Google Auth Platform`。
...
> 进入 Google Auth Platform 页面后，如果看到 Get Started，点击它。
> 
> 填写 `App name`，例如 rclone；
> 
> `User support email` 选择你自己的 `Google` 邮箱；
> 
> Audience 如果是普通 Gmail/个人 Google 账号，选 `External`。如果你是单位自己的 Google Workspace，并且只允许本单位账号使用，才考虑 Internal。Google 官方说明 External 可供 Google Account 用户授权，Internal 则限组织内部。
...
进入 `Google Auth Platform → Data Access → Add or remove scopes`，配置 `OAuth Scopes`。按照 rclone 当前官方说明，推荐加入：
...
它允许 rclone 对 Google Drive 中的文件进行`读取、创建、修改和删除`。
...
- 进入 `Google Auth Platform → Branding（品牌塑造）`，重点把带 * 的必填项补齐。

> App name：自定义，如rclone 或 rclone-drive。
>
> User support email：选择你自己的 Google 邮箱。
>
> App logo：可以不上传，建议暂时留空。
>
> Application home page：必填，如`https://rclone.org/`。
>
> Privacy policy：必填，如`https://rclone.org/privacy/`。
>
> Terms of service：可以留空。
>
> Authorized domains：必填，如`rclone.org`。
>
> Developer contact information / Contact email：填写你自己的 Google 邮箱。
...
1. 进入 `Google Auth Platform → Audience/目标对象`，把自己的 Google 账号加入测试用户。 

> 进入 Audience，找到 Test users，点 Add users，把你之后准备让 rclone 登录的 Google Drive 账号加进去。
> 
> 需要注意，如果 OAuth 应用一直处于 Testing 状态，Google 对这类非基础 OAuth scope 签发的 refresh token 通常只有 7 天有效期，这意味着服务器上的 rclone 大约每周就可能需要重新授权。
...
2. 发布状态：不要选`Testing 测试`（测试状态下api有有效期仅7天），建议`publish app 发布应用`；用户类型选`外部`；添加用户为自己的谷歌邮箱账号。

> 对于你这种个人 rclone 使用，建议`publish app 发布应用`。 在 Audience 页面找到 Publish App，将状态从 Testing 改成 In Production / Production。
> 
> 你自己使用、用户远少于 100 人的情况下，Google 官方明确将 Personal Use apps 列为可以不进行完整 OAuth Verification 的场景；登录时可能看到“Google hasn't verified this app / 未验证应用”的警告，但可以继续授权。
> 
> 这样做最重要的好处是避免 Testing 状态下 refresh token 7 天过期的问题。



### 5. 创建 OAuth client ID

进入 `Google Auth Platform → Clients`，点击 `Create Client`。`Application type` 一定选择：`Desktop app`。点击 Create，Google 会生成`Client ID和Client Secret`。

1. 创建 OAuth 客户端 ID

可以跳过 Credentials/凭据，直接进入 `Google Auth Platform → Clients`，点击 `Create Client`。
...
```sh
# 出现大文件传输、网络抖动或网盘 API 限速时，同步时间极易超过 5 分钟。未加锁会导致第 5、10、15 分钟不断启动新的 rclone 实例。
*/5 * * * * /usr/bin/rclone sync 'rc4:/3图书/13_paperRemoteStorage/' 'gd1:/13_paperRemoteStorage/' --transfers=16

# flock -n 采用非阻塞排他锁。如果上一轮同步尚未结束，当前实例获取锁失败会立即静默退出，彻底避免了进程雪崩与数据读写冲突。
*/5 * * * * flock -n /tmp/rclone_book.lock /usr/bin/rclone sync 'rc4:/3图书/' 'gd3:/02_bookSyncOnedrive/' --transfers=8
```


- 查看`rclone.conf`配置文件中有关`google drive`核心配置信息的命令
...
注意：`client_id`、`client_secret`和`token`三个参数需要重点修改，注意`team_drive`为空，不需要设定任何值。
...
然后执行如下命令，注意授权的是`drive`（对应`google drive`），不是`onedrive`；使用 `--auth-no-open-browser` 参数，可以强制 rclone 不自动打开浏览器，而是直接在命令行中输出授权链接，然后你手动复制该链接到 Chrome 中打开。

```sh
rclone authorize "drive" "959a.........68111" "P8S8............P.AWJbqW" --auth-no-open-browser
```
