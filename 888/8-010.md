...
进入 Google Auth Platform 页面后，如果看到 Get Started，点击它。填写 `App name`，例如 rclone；`User support email` 选择你自己的 `Google` 邮箱；Audience 如果是普通 Gmail/个人 Google 账号，选 `External`。如果你是单位自己的 Google Workspace，并且只允许本单位账号使用，才考虑 Internal。Google 官方说明 External 可供 Google Account 用户授权，Internal 则限组织内部。
...
把自己的 Google 账号加入测试用户。 进入 Audience，找到 Test users，点 Add users，把你之后准备让 rclone 登录的 Google Drive 账号加进去。
